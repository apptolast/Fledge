...
interface MoneyFlowRepository {
    ...
    suspend fun saveAllowanceRule(draft: AllowanceRuleDraft, nextRunAt: Instant, createdAt: Instant): AllowanceRule

    suspend fun updateAllowanceNextRun(ruleId: AllowanceRuleId, nextRunAt: Instant, updatedAt: Instant): AllowanceRule

    fun dueAllowanceRules(now: Instant): List<AllowanceRule>

    fun allowanceRuleById(ruleId: AllowanceRuleId): AllowanceRule?

    suspend fun createSettlementRequest(draft: CashOutSettlementDraft, requestedAt: Instant): CashOutSettlement

    suspend fun markSettlementPaidByParent(settlementId: SettlementId, paidAt: Instant): CashOutSettlement
