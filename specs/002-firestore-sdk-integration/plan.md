...
| **T7** | Xcode/SPM: `firebase-ios-sdk` pinneado a 11.8.x, productos `FirebaseCore` + `FirebaseFirestore` en el target `iosApp`, `Package.resolved` versionado | AC-02 | **Paso manual del usuario** (decidido en Gate 2): lo añade desde Xcode y el agente commitea el resultado y valida con `xcodebuild`. Ver riesgos |
...
| **Editar `project.pbxproj` a mano para el SPM** | *mitigado* | ✅ **Resuelto en Gate 2**: no se edita a mano. El usuario añade el paquete desde Xcode (File → Add Package Dependencies → `https://github.com/firebase/firebase-ios-sdk`, regla *Up to Next Minor* desde 11.8.0, productos `FirebaseCore` y `FirebaseFirestore` sobre el target `iosApp`) y el agente commitea `project.pbxproj` + `Package.resolved` y valida con `xcodebuild`. T7 queda como **paso bloqueante manual** dentro de `/implement` |
